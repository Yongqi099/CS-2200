...
packet_t *packetize(char *buffer, int length, int *count) {
    ...
    int num_packets = length / MAX_PAYLOAD_LENGTH, last_payload_length = MAX_PAYLOAD_LENGTH;

    if (length % MAX_PAYLOAD_LENGTH) {
      last_payload_length = length % MAX_PAYLOAD_LENGTH;
      num_packets++;
    }

    packet_t *packets = malloc(num_packets * sizeof(packet_t));

    // Populate all the fields of each packet except for the last one
    for (int i = 0; i < num_packets - 1; i++) {
        // Set the type of all packets except for the last one to DATA
        packets[i].type = DATA;
        // Set the payload length to MAX_PAYLOAD_LENGTH for all packets except for the last one
        packets[i].payload_length = MAX_PAYLOAD_LENGTH;
        // Copy the payload data from the buffer to each packet
    ...
    // Populate all fields of the last packet
    packets[num_packets - 1].type = LAST_DATA;
    packets[num_packets - 1].payload_length = last_payload_length;
    memcpy(packets[num_packets - 1].payload, buffer + (num_packets - 1) * MAX_PAYLOAD_LENGTH, last_payload_length);
    packets[num_packets - 1].checksum = checksum(packets[num_packets - 1].payload, last_payload_length);

    // Update the count variable to be equal to the length of the array
    *count = num_packets;
    return packets;
}
    ...
    do {
        ...
        do {
            ...
            */
            ...
            // Check if the packet received is a data packet
            if (packet.type == LAST_DATA || packet.type == DATA) {
                
                // Allocate memory for an ACK or NACK packet
                packet_t *ack_packet = malloc(sizeof(packet_t));

                // Check if the payload checksum matches the packet checksum
                if (checksum(packet.payload, packet.payload_length) == packet.checksum) {

                    // If the checksums match, send an ACK
                    ack_packet->type = ACK;
                    
                    // Reallocate memory to fit the new payload
                    // If buffer is NULL, realloc behaves like malloc and allocates new memory
                    buffer = realloc(buffer, buffer_length + packet.payload_length);

                    // Copy the payload into the buffer and update the buffer length
                    memcpy(buffer + buffer_length, packet.payload, packet.payload_length);
                    buffer_length += packet.payload_length;

                } else {
                    // If the checksums do not match, send a NACK
                    ack_packet->type = NACK;
                    // Change LAST_DATA to DATA to prevent the loop from terminating
                    if (packet.type == LAST_DATA) packet.type = DATA;
                }

                // Send the ACK or NACK packet
                net_send_packet(connection->net_connection_handle, ack_packet);
                // Free the memory allocated for the ACK or NACK packet
                free(ack_packet);
            } else if (packet.type == ACK || packet.type == NACK) {
                // Check if the packet received is an ACK or NACK packet

                // Lock the mutex to ensure thread-safe access to the connection's ack value
                pthread_mutex_lock(&connection->ack_mutex);

                // Set the connection's ack value based on the packet type
                if (packet.type == ACK) connection->ack = ACK;
                else connection->ack = NACK;

                pthread_mutex_unlock(&connection->ack_mutex);
                // Signal the condition variable to notify the sending thread that an ACK or NACK has been received
        ...
        if (connection->alive == 1) {
            ...
            */
            ...
            // Add the message variable to the rtp client's queue in a thread-safe manner
            ...
            // Signal the recv_cond condition variable to let the client know a full message has been received
            pthread_cond_signal(&connection->recv_cond);
...
static void *rtp_send_thread(void *void_ptr) {
    ...
    do {
        ...
        for (i = 0; i < array_length; i++) {
            ...
            // Wait for an ACK or NACK to be received
            while (!connection->ack)
            pthread_cond_wait(&connection->ack_cond, &connection->ack_mutex);

            // If a NACK was received, decrement i to resend the last packet
            if (connection->ack == NACK) i--;
            // Reset the connection's ack value to DATA
            connection->ack = DATA;
