...
void proc_init(pcb_t *proc) {
    ...
    pfn_t pageTable = free_frame();                     //allocate frame for process table
    memset(mem + pageTable * PAGE_SIZE, 0, PAGE_SIZE);  //clear space for process
    ...
    fte_t * frame = (frame_table + pageTable);
    frame->protected = 1;
    frame->mapped = 1;
    frame->process = proc;
}
